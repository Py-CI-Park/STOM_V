...
class KiwoomStrategyTick:
    ...
    def Strategy(self, data):
        ...
        if 종목코드 in self.dict_arry:
            ...
            if len_array >= 평균값계산틱수 - 1:
                ...
                등락율각도_      = round(math.atan2((등락율 - self.dict_arry[종목코드][-(평균값계산틱수 - 1), 5]) * dgree['stock']['tick'][0], 평균값계산틱수) / (2 * math.pi) * 360, 2)
    ...
    def SetSellCount(self, 분할매도횟수, 보유수량, 매입가, 저가대비고가등락율, 순매수금액, 당일거래대금, 등락율각도):
