...
### 1.1 목적
STOM_Version_1(V1) 코드베이스를 기반으로, STOM_Version_2(V2)에서 순차적으로 개발된 33개 커밋(V2.00 ~ V2.33)의 변경사항을 **코드를 이해하고 추론하여** STOM_Version_1U 브랜치에 체계적으로 반영한다.

### 1.2 핵심 원칙
- **체리픽(cherry-pick) 금지**: 단순 커밋 복사가 아닌, 각 커밋의 의도와 코드 변경을 이해한 뒤 V1U 코드베이스에 맞게 재구현
- **순차적 개발**: V2.00부터 V2.33까지 한 단계씩 진행하며, 각 단계에서 컴파일/실행 가능한 상태 유지
...
| 참조 범위 | `b021269` (V2.00) ~ `d7997af` (V2.33), 총 33개 커밋 |
...
이 문서는 STOM_Version_1U 브랜치에서 V2의 33개 커밋을 **이해 기반으로 재구현**하기 위한 완전한 개발 계획서입니다.
...
- **34단계 순차 개발**: 각 단계마다 독립적으로 검증 가능한 커밋 생성
