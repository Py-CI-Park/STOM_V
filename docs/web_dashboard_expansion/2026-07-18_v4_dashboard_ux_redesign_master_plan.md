...
## 11. 재검토 체크리스트 (사장님 원본 피드백 정직 대조 · 2026-07-18)

> 직접 3440×1440 실측 재검토 결과: 인프라·정확성(세션 4401·IA·Reports·카탈로그)은 완료됐으나
> **사장님 최우선 요구인 Live 탭 시각 재배치는 미완**. FITNESS 곡선 단일 그래프가 2780×520px 로
> 전체를 지배하고, Live 탭 전체 높이 18,136px(12.6화면 스크롤), 첫 화면에 그래프 4개만 노출.
> P4 "울트라와이드"는 사이드 컬럼 폭(352→480)만 넓혔을 뿐 레이아웃 자체 미개선. 아래 표로 정직 기록.

### 11.1 전역 요구
| # | 요구 | 상태 | 근거/격차 |
|---|---|---|---|
| G1 | Audit 패널 불필요 확인·제거 | ✅ 완료 | F1 Audit 탭 retire·거버넌스 History 이전 |
| G2 | 오른쪽 패널 펼쳐 확대·글자 확대 | 🟡 부분 | 사이드 480px·타이포 breakpoint. "펼쳐 확대" 인터랙션·본문 확대 미흡 |
| G3 | 버전 글자 크게+하이라이트 효과 | 🟡 부분 | build 배지 추가. "크게+애니메이션/효과 버튼" 미흡 |
| G4 | 좌측 탭 확대·글자 가독·version 효과 버튼 | 🟡 부분 | 탭 좌측 액센트·울트라 타이포. 애니메이션 효과 버튼 미구현 |
| G5 | 연구보고서 체계적 HTML(전체+스텝별 상세·탭 이동·결과 예시) | 🟡 부분 | Reports 허브(기존 HTML 열람). **스텝별 상세 자동생성·탭 이동 미구현** |
| G6 | 울트라와이드 전체 재배치 | ❌ **미완(핵심)** | Live 레이아웃 그대로·거대 단일 그래프 |
| G7 | HTML 보고 시스템 | ✅ 완료 | Reports 허브(CSP+sandbox) |
| G8 | Wiki 문서 체계 | ❌ 미착수 | — |
| G9 | 3440에서 그래프 과대·탭제목/버튼 확대 | ❌ **미완** | 그래프 2780×520 그대로. 탭제목 일부만 |
| G10 | 연결 끊김 재연결 반복 | ✅ 완료 | 세션 4401 근본수정(22→0) |
| G11 | 프로세스 단계별 사용자와 함께 진행(활성화) | 🟡 부분 | P5 상태기계. "단계별 탭 자동전환" 미구현 |

### 11.2 Live 탭 (대부분 미완 — 다음 최우선)
| # | 요구 | 상태 | 격차 |
|---|---|---|---|
| L1 | 전체 모니터 재배치·한 화면 많은 정보 | ❌ **미완(핵심)** | 18,136px 스크롤·첫 화면 4그래프 |
| L2 | 글만 있는 블록(단계시간/차단/로그) ux 개선 | ❌ 미개선 | 텍스트 나열 그대로 |
| L3 | 반복세대 사이클·현재세대 정리+애니메이션 | ❌ 미완 | 상단+우측 분산 |
| L4 | 프로세스별 탭+진행시 자동전환+시각화 | ❌ 미착수 | — |
| L5 | 백테스트 엔진 상태 상단 대시보드 정리 | ❌ 미착수 | — |
| L6 | 백테 단계 조건식 노출·분석 시각화 | ❌ 미착수 | — |
| L7 | 불필요 프로세스 병합·버튼/내용 축소 | ❌ 미완 | "한눈 흐름" 등 과다 |
| L8 | 기본설정/게이트/채점기준 클릭 시 표시 | ❌ 미착수 | — |
| L9 | History 고도화(리스트→상세 아키텍처) | 🟡 부분 | F2 잘림노출만. 상세 아키텍처 미완 |
| L10 | 깜빡이는 연결끊김 메시지 | ✅ 완료 | 세션 수정 |

### 11.3 Lab/Workbench/Context
| # | 요구 | 상태 | 격차 |
|---|---|---|---|
| W1 | Lab → Live 백테 분석결과에 통합 | ❌ 미착수 | — |
| W2 | Lab 시간대/시총 중복·워크벤치 중복 통합 | ❌ 미착수 | — |
| W3 | 워크벤치=명예의전당만·탭 개명 | 🟡 부분 | "성과" 개명 O. 명예의전당만 축소 미완 |
| W4 | Audit 불필요 | ✅ 완료 | F1 |
| W5 | 알파랩 추후 함께 고려 | ✅ 반영 | 문서 |
| W6 | Context 재검토(가독성·존재이유) | 🟡 부분 | 보조군 격하. 개선/제거 판단 미완 |

### 11.4 Backtest — D2 사장님 결정 반영
- 요구 B1: 백테스트 **독립 유지** + python-GUI 백테를 대시보드에서 실행 + **강력한 결과 시각화**.
- **사장님 D2 답변 = "3 추천, 4 채택"**: 추천 순서(3)는 참고하되 **강력 시각화는 "UX 개선 후"(옵션 4)** 에 수행.
  → **순서 확정: Live/울트라와이드 UX 재배치(아래 §11.5) 선행 → 그 다음 Backtest 강력 시각화 강화.**
- 현재 상태: 독립 유지 O·divid_mode GUI 패리티 O. 강력 시각화는 UX 개선 후(대기).

### 11.5 다음 최우선 실행 — Live/울트라와이드 재설계 (미완 핵심 집중)
실측 근거로 아래를 **다음 단독 대작업**으로 브랜치→개발→라인연결 merge:

1. **거대 단일 그래프 해체 → 밀도 그리드**: hero 2780×520 축소, 3440에서 2×2/3열 그래프 그리드로 여러 지표 동시 노출(스크롤 18k→1~2화면 목표).
2. **텍스트 블록 → 카드/시각 컴포넌트**: 단계별시간·차단사유·최신로그(L2)를 접이식 카드·배지·미니 타임라인으로.
3. **프로세스 스테퍼 중심 상단 상황판**(L3·L5): 반복세대 사이클·현재세대·백테엔진 상태를 상단 단일 상황판으로 통합, 애니메이션.
4. **단계별 뷰 + 진행시 자동전환**(L4·G11): 생성/백테/채점/부검 각 단계 뷰, 라이브 단계 전환 시 자동 포커스(user-pinned 존중).
5. **정보 계층화·클릭 노출**(L7·L8): 기본설정/게이트/채점기준을 상시 노출 대신 클릭 시 상세, 불필요 프로세스 병합.
6. **탭 제목·버튼 타이포 확대**(G9): 각 탭 상단 제목·주요 버튼 크기 상향.
7. 검증: 3440/2560/1080 3해상도 실측(첫 화면 그래프 수·스크롤 높이 before/after 수치 기록).

### 11.6 남은 페이지(뷰) 요약
- **최우선**: §11.5 Live/울트라와이드 재설계(가장 큰 미완).
- P4 카탈로그 잔여 3뷰(함정지도/절실험실/출구은행) + B1 scorecard.
- G5 스텝별 HTML 리포팅·G8 Wiki 체계·W1~W3 Lab/Workbench 통합·W6 Context 판단.
- 이후 Backtest 강력 시각화(D2 순서).
- mainline 라인연결 반환 PR.
