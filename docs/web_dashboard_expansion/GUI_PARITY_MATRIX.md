...
| 전진분석(WFO) | `전진분석` · `train/valid/test_period` | `stom_backtest wfo`(CLI 존재, 웹 잡 래핑 미연결) | ⛔ (다음 단계) |
| 파라미터 스윕/날짜 롤링 | `일괄변경` · 스케쥴 | `stom_backtest sweep param/rolling`(CLI 존재) | ⛔ (다음 단계) |
...
1. **WFO 잡 래핑** — `stom_backtest wfo` 를 잡 매니저 mode 로 추가(`mode="wfo"`, train/test 윈도우 입력). CLI 계약은 `cli/subcommands.py:342` 참조.
2. **스윕 잡 래핑** — `sweep param/rolling` mode 추가(params JSON · window/step 입력).
3. **백파인더 진입점** — `discovery auto/analyze/generate` 를 잡 타입 확장으로 노출.
4. **param_space 인라인 편집기** — JSON 경로 대신 웹에서 탐색공간을 폼으로 정의 → state/ 하위에 직렬화 후 실행.
5. **수식(formula) 전용 에디터** — 듀얼 에디터에 formula 탭 추가(차트표시/색상 등 메타 포함).
