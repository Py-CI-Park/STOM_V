...
9. [V2.59~V2.65 CLI_v258 동기화 실전 사례](#9-v259v265-cli_v258-동기화-실전-사례)
10. [V2.58 파일 구조 차이 상세](#10-v258-파일-구조-차이-상세)
11. [FAQ](#11-faq)
...
│  3. 브랜치 특성에 따라 오버레이 또는 cherry-pick 선택               │
│  4. pyd 추론은 버전 단위로 수행 (pyd 변경이 있는 버전만)           │
│  5. V2 → 2U → 2U_C → CLI 순서로 한 단계씩 전파                    │
└──────────────────────────────────────────────────────────────────────┘
```

### 4.2 오버레이 vs cherry-pick 사용 기준

**브랜치가 업스트림 파일을 커스텀 수정했는지**에 따라 방식이 달라집니다.

```
업스트림 파일을 수정하지 않는 브랜치 → 오버레이 (파일 통째 복사)
업스트림 파일을 커스텀 수정한 브랜치 → cherry-pick (변경분만 병합)
```

| 브랜치 | 업스트림 파일 커스텀 수정 | 방식 | 이유 |
|--------|-------------------------|------|------|
| `V2` | 없음 (원본) | **오버레이** | 업스트림 그대로 수용 |
| `2U` | ui_mainwindow.py만 (pyd→py) | **오버레이** | pyd 제외 후 py만 보존 |
| `2U_C` | 일부 (설정, 리팩토링) | **cherry-pick** | 커스텀 수정 보존 필요 |
| `CLI_v258` | 다수 (backtest/, utility/) | **cherry-pick** | CLI 코드 보존 필수 |

**오버레이를 커스텀 브랜치에 사용하면?**
→ 커스텀 수정이 업스트림 버전으로 덮어씌워져 기능이 깨집니다.
(실제 사례: CLI_v258에서 66개 테스트 실패 → cherry-pick으로 전환 후 해결)

### 4.3 방식별 상세 비교

| 방식 | 원리 | 장점 | 단점 | 적합한 브랜치 |
|------|------|------|------|-------------|
| **오버레이** | devstom export → 파일 통째 복사 | 단순, 빠름, 정확 | 커스텀 수정 덮어씀 | V2, 2U |
| **cherry-pick** | 2U 버전 커밋의 변경분만 적용 | 커스텀 보존, 3-way merge | 충돌 수동 해결 필요 | 2U_C, CLI_v258 |
| `git merge` | 브랜치 간 자동 병합 | 자동 | V2.58 baseline 불일치 | **사용 불가** (V2↔devstom) |

### 4.4 cherry-pick 전파 체인
...
devstom (업스트림 원본)
    ...
    ▼
V2 (버전별 오버레이 커밋)
    │ git archive → 오버레이 (pyd 제외)
    ▼
2U (버전별 오버레이 커밋 + pyd 추론)
    │ git cherry-pick (2U의 버전 커밋)
    ▼
2U_C (cherry-pick + 커스텀 보존)
    │ git cherry-pick (2U_C의 버전 커밋)
    ▼
CLI_v258 (cherry-pick + CLI 커스텀 보존 + 테스트 검증)
```
...
_update.txt에 새 버전이 추가됨?
    ...
    │
    └── 예 → 동기화 시작 (각 버전별로 순차 반복)
         │
         ├── 1. V2: 해당 버전 오버레이
         ├── 2. 2U: 해당 버전 오버레이 (pyd 제외 + 추론)
         ├── 3. 2U_C: 2U의 해당 버전 커밋 cherry-pick
         └── 4. CLI_v258: 2U_C의 해당 버전 커밋 cherry-pick + 테스트
```
...
| 새 버전 발견 | 즉시 또는 다음 작업 전 | 버전 오버레이(V2,2U) + cherry-pick(2U_C,CLI) |
| 급한 버그 수정 | 즉시 | 해당 커밋만 cherry-pick 가능 |

---

## 9. V2.59~V2.65 CLI_v258 동기화 실전 사례

### 9.1 cherry-pick 충돌 해결 기록

CLI_v258에 2U_C의 V2.59~V2.65 cherry-pick 시 발생한 충돌과 해결 방법:

| 버전 | 충돌 파일 | 원인 | 해결 |
|------|----------|------|------|
| V2.59 | `utility/setting.py` | 업스트림 삭제 vs CLI 수정 | CLI 보존 (CLI import 필수) |
| V2.59 | `backtest/back_subtotal.py` | 양쪽 수정 | CLI 보존 (ours) |
| V2.59 | `backtest/backengine_base.py` | 양쪽 수정 | CLI 보존 (ours) |
| V2.59 | `backtest/backengine_base_oms.py` | 양쪽 수정 | CLI 보존 (ours) |
| V2.60 | `backtest/backengine_base.py` | 양쪽 수정 | CLI 보존 (ours) |
| V2.61 | `backtest/backengine_base.py` | 양쪽 수정 | CLI 보존 (ours) |
| V2.62 | `backtest/` 6파일 + `utility/static.py` | 양쪽 수정 | CLI 보존 (ours) |

### 9.2 cherry-pick 후 추가 수정 사항

cherry-pick 완료 후 테스트(772개)로 발견한 호환성 문제와 수정:

| 문제 | 원인 | 수정 |
|------|------|------|
| `utility/lazy_imports.py` 누락 | V2.62에서 삭제됨, CLI backtest.py가 import | CLI 버전으로 복원 |
| `DB_STOCK_BACK_TICK` 미정의 | `setting_base.py`가 업스트림 버전(다른 이름)으로 교체됨 | CLI 호환 별칭 6개 추가 |
| `thread_decorator` SystemExit 동작 변경 | V2.62 `utility/static.py`가 SystemExit catch | 테스트 수정 (pytest.raises 제거) |

### 9.3 충돌 해결 원칙 (CLI 브랜치)

```
1. CLI가 커스텀 수정한 파일 → CLI 버전 유지 (--ours)
2. CLI가 수정하지 않은 파일 → 업스트림 자동 반영
3. cherry-pick 후 반드시 pytest 실행하여 호환성 확인
4. import 에러 발생 시:
   a. 삭제된 모듈 → CLI에서 필요하면 복원
   b. 이름 변경된 변수 → 호환 별칭 추가
   c. 동작 변경된 함수 → 테스트 수정
```

---

## 10. V2.58 파일 구조 차이 상세
