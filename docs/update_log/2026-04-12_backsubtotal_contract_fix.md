...
- `research/init`에는 CLI 통합 과정에서 `BackSubTotal.__init__`이 `windowQ` 없이 축약된 계약으로 남아 있었다.
...
- 이번 문제는 공식 `STOM_Version_2` 업데이트 자체의 오류가 아니라, 공식 UI 계약과 CLI 계열 축약 생성자 계약이 `2U_C` 및 `research/init` 계열에서 혼재되며 발생한 통합 불일치다.
