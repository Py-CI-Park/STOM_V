...
| 시스템이 완성됐는가? | **아니다. 구현 성숙도 약 6.0/10.** 거래 경로 복원과 기본 반사실은 되었지만, 정확한 STOM 매도식 재생과 복합 조건 생성은 아직 얕다. |
...
| 가장 큰 부족함은? | 현재 QSP7 매도 후보 생성기가 tick/min별 2개 템플릿 수준이며, 사람 전략 DB와 풍부한 프롬프트·STOM 변수 체계에 연결되지 않았다. |
| 바로 다음 핵심은? | **P1 데이터 보존 → P2 정확한 매도식 인터프리터 → P3 tick/min별 복합 후보 생성기 → P4 공식 설계/OOS 게이트** 순서다. |
...
| 현재 구현 성숙도 | **6.0/10** | 경로 복원과 기본 UI는 있으나 정확한 매도 DSL 재생·복합 후보 탐색·OOS 자동 게이트가 미완성이다. |
| 데이터 정밀도 | **6.5/10** | 거래 시각·가격·사유와 경로는 결합하지만 B_/S_/R_ 스냅샷, 종목코드, 분할 체결, 정확한 비용 원장이 충분히 보존되지 않는다. |
| tick/min 분리 수준 | **6.0/10** | DB·시각·보유시간 단위는 분리했다. 후보 정책의 구조는 아직 거의 동일하다. |
| 조건식 생성 품질 | **QSP7 3.0/10** | 실제 QSP7은 2개 결정론 템플릿이다. 기존의 풍부한 프롬프트와 사람 전략 DB를 사용하지 않는다. |
| 공식 검증 연결 | **6.0/10** | 공식 control·pair·매트릭스 경로는 연결됐다. 변경 후보의 설계/OOS 실행은 아직 없다. |
...
| 4 | 왜 빨리/늦게 팔았는지 분석 | **6/10** | 부분 구현 | 고정 horizon·회복 여부를 넘어 시간-대-회복 곡선과 반증 비교 필요 |
| 5 | 여러 매도 가설 가상 재생 | **4/10** | 기본 정책만 구현 | 실제 STOM 식, `if/elif` 우선순위, 다양한 조건군의 first-trigger 재생 필요 |
| 6 | 사람이 이해할 조건식 후보 생성 | **3/10** | 초안 2개 | 사람 전략 DB·복합 예시·코호트 변수·다양한 후보군 연결 필요 |
| 7 | 공식 백테스트 재검증 | **6/10** | 실행 경로 구현 | 변경 매도식 설계 구간을 실제 실행하고 pair 전이를 확인해야 함 |
| 8 | 설계/OOS 동시 통과만 채택 | **3/10** | 원칙만 설계 | 잠긴 OOS, 채택 점수표, 실패 기록, 재현성 게이트 구현 필요 |
