...
| 레인 우선순위 | **tick 먼저**(최신 2년 · 런 1회 9분) → min 나중 |
| **평가 방식** | **v2: 최신 2년 단일 연속 런 + CSV 날짜 분할** (후보당 백테 2회 → **1회**) |
| 기간 | 3.5~4일 (엔진 1.5일 + v2 배선 0.5일 + 화면 1일 + 세대 루프 2.5시간 + 보고 0.5일) |
...
| **G-1** | tick 기준선 1회(대시보드) + 1세대 후보 3~4개 | 홀드아웃 동반 개선 · 예산 준수 | 9분 + 36분 |
| **G-2~G-4** | tick 2~4세대 | 〃 | 세대당 ~36분 |
...
| 11 | 엔진 수를 늘려도 빨라지지 않는다(64 = 16) — **런 1회 9분**을 전제로 계획한다 |
...
- 엔진은 64를 사용하되, 엔진을 늘려도 빨라지지 않습니다(실측 64=16). 런 1회 ≈ 9분.
...
- G-1: 대시보드로 tick 기준선 1회 실행(9분) → 1세대 후보 3~4개(각 1회 런)
- G-2~G-4: 2~4세대 (세대당 약 36분)
