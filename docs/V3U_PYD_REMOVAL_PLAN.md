...
V3U pyd 추론 과정에서 발견된 모든 결함, 근본 원인, 재발 방지 액션은 `docs/V3U_INFERENCE_LESSONS.md`에 누적 기록된다. 본 문서는 lane 종료 시까지 갱신되는 진실 원천이며, 모든 결함 수정 커밋은 그 문서 §6 결함 기록을 함께 갱신해야 한다 (4단계 워크플로우, `CLAUDE.md` "결함 발견·수정 4단계 워크플로우" 섹션 참조).
