...
- 계획서의 decision rule을 그대로 적용하면 `status=ok`이고 `control_reference_adjusted_score`가 비어 있어 `PASS_TO_V3_EXECUTION_RESULT_ANALYSIS`로 분기된다.
...
- top 10 후보가 모두 동일한 `reference_adjusted_score=13497.662902097409`로 tie였고, `cleanup_summary`는 loser 9개 삭제와 `cand001` 유지로 끝났다.
