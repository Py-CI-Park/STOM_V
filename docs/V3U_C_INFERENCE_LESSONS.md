...
### 사이클 8 (2026-07-12): V3.35 흡수 (V3U lane 따라잡기) — 결함 0건

- 산출: merge commit `ff704397` (`git merge --no-ff STOM_Version_3U`)
- 흡수: V3.35 (바이낸스선물 정정주문, LS 시장가 주문가격 수정, 주문 예외 처리 강화) + tail 4건 (ordxctptncode 방어, 해외선물 체결/정정취소 분리 등)
- 테스트: tests/v3u 49 + tests/v3uc 32 = 81 PASS, 통합 게이트 8/8, lane V3.35
...
| 측정 | 값 (사이클 8 종료 시점, 2026-07-12) |
|---|---|
| 총 발견 결함 | 2 (#1 argparse parents, #2 cp949 인코딩) — 사이클 5~8 신규 0건 |
| 자동 회귀 테스트 | 3U_C custom 32 + V3U 안전망 상속 49 = 81 |
| 신규 자동 도구 | 4 (ingest_pipeline, db_compatibility_check, strategy_migration, cli) |
| 신규 문서 | 6 + 흡수 감사 3 (V3U V3.33/V3.34/V3.35 pyd-free update logs 상속) |
