...
from utility.setting import DB_STRATEGY, DICT_SET, ui_num, columns_jg, columns_gj, dict_order_ratio, DB_COIN_TICK, DB_COIN_MIN, indicator, dgree
...
class UpbitStrategyTick:
    ...
    def SetBuyCount(self, 분할매수횟수, 매입가, 현재가, 고가, 저가, 등락율각도, 당일거래대금각도):
        ...
        매수수량 = round(betting / (현재가 if 매입가 == 0 else 매입가) * oc_ratio / 99.99, 8)
        return 매수수량
    ...
    def SetSellCount(self, 분할매도횟수, 보유수량, 매입가, 현재가, 고가, 저가, 등락율각도, 당일거래대금각도):
        if self.dict_set['코인매도분할횟수'] == 1:
            ...
            매도수량 = round(betting / 매입가 * oc_ratio / 99.99, 8)
    ...
    def PutGsjmAndDeleteHilo(self):
        self.df_gj.sort_values(by=['dm'], ascending=False, inplace=True)
