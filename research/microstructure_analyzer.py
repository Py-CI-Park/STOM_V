...
class MicrostructureAnalyzer:
    ...
    def _analyze_risk(self, code: str):
        ...
        total_risk        = (market_risk + manipulation_risk + liquidity_risk + price_risk) / 4     # 총 리스크 (평균)
    ...
    def _calculate_price_risk(self, code: str) -> float:
        ...
        """
        data_buffers = list(self.data_buffers[code])
        array_data   = get_np().array(data_buffers)
    ...
    def _calculate_returns(self, prices: get_np().ndarray) -> get_np().ndarray:
        """가격 데이터로부터 수익률 계산"""
        return get_np().diff(prices) / prices[:-1]

    def _calculate_var_historical(self, returns: get_np().ndarray, confidence: float = 0.95):
        """VaR 계산"""
        return -get_np().percentile(returns, (1 - confidence) * 100)

    def _calculate_sharpe_ratio(self, returns: get_np().ndarray, annualize: bool = True):
        """Sharpe Ratio 계산"""
        ...
        return sharpe * get_np().sqrt(250) if annualize else sharpe

    def _calculate_max_drawdown(self, prices: get_np().ndarray):
        """최대낙폭 계산"""
