...
from research.microstructure_analyzer import MicrostructureAnalyzer
...
def example_realtime_simulation(market_type: str = 'stock', buy_cfd_limit: float = 0.6, sell_cfd_limit: float = 0.6):
    ...
    try:
        ...
        ms_analyzer = MicrostructureAnalyzer(market_type, test_mode=True)
        ...
        for i, row in enumerate(data_array):
            ...
            # 미시구조 분석 신호 생성
            ms_analyzer.update_data(selected_stock, row)
            signal, confidence = ms_analyzer.get_signal(0.0, 0.0)
        ...
        # 분석 범위
        buy_flows   = [data[0] for data in ms_analyzer.data_results]
        sell_flows  = [data[1] for data in ms_analyzer.data_results]
        cinfidences = [data[2] for data in ms_analyzer.data_results]
