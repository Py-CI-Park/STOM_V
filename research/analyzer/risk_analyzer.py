...
class RiskAnalyzer:
    ...
    def calculate_var_historical(self, returns: np.ndarray, confidence: float = 0.95):
        """
        역사적 VaR 계산 (속도 우선: np.percentile)
        """
    ...
    def calculate_sharpe_ratio(self, returns: np.ndarray, annualize: bool = True):
        """
    ...
    def calculate_max_drawdown(self, prices: np.ndarray):
        """
    ...
    def analyze_portfolio_risk(self, prices_dict: Dict[str, np.ndarray]) -> Dict:
        """
        포트폴리오 리스크 분석 (속도 우선: 행렬 연산)
        """
        codes = list(prices_dict.keys())
        returns_matrix = np.array([self.calculate_returns(prices) for prices in prices_dict.values()])
    ...
    def calculate_skewness(self, returns: np.ndarray):
        """
        왜도 계산 (속도 우선: np 함수)
        """
        return np.mean(((returns - np.mean(returns)) / np.std(returns, ddof=1)) ** 3)
    ...
    def calculate_kurtosis(self, returns: np.ndarray) -> float:
        ...
        """
        # noinspection PyTypeChecker
        return np.mean(((returns - np.mean(returns)) / np.std(returns, ddof=1)) ** 4) - 3

    def risk_report(self, analysis_results: Dict) -> pd.DataFrame:
        """
        리스크 분석 리포트 생성
        """
        individual_results = {k: v for k, v in analysis_results.items() if isinstance(v, dict)}
        df = pd.DataFrame.from_dict(individual_results, orient='index')
        df = df.round(4)
        return df
...
if __name__ == "__main__":
    # 샘플 데이터 생성 (속도 테스트용)
    np.random.seed(42)
    sample_prices = np.random.randn(1000).cumsum() + 100  # 랜덤 가격 데이터
    ...
    _prices_dict = {
        ...
        'Stock2': sample_prices * 1.1 + np.random.randn(1000) * 0.5
    }
