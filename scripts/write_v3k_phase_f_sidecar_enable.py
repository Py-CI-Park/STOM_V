...
from strategy.v3k_analyzer_adapter import FLAG_PHASE_F_ANALYZER_STRATEGY  # noqa: E402
...
USER_ACK_ENV = "V3K_PHASE_F_USER_ACK"
PHASE_F_GATE = "phase-f-f4-on-await-user-approval"
PHASE_F_PHRASE = "I approve phase-f-f4-on-await-user-approval only"
WRITER_VERSION = "V3K_PHASE_F_SIDECAR_ENABLE_WRITER_V1"
ALLOWED_ENABLED = {FLAG_PHASE_F_ANALYZER_STRATEGY}
...
def _assert_forbidden_status_clean() -> None:
    ...
    if status:
        raise SystemExit(f"forbidden runtime/DB artifact status is not clean before Phase F enable:\n{status}")
...
def _assert_gate_authority(approval_phrase: str) -> None:
    ...
    if not verdict.accepted or verdict.gate != PHASE_F_GATE:
        raise SystemExit(f"approval phrase rejected for Phase F gate: {verdict.status} / {verdict.reason}")
    if os.environ.get(USER_ACK_ENV) != "1":
        raise SystemExit(f"{USER_ACK_ENV}=1 is required for Phase F approved gate execution")


def _load_base_payload(target: Path) -> dict[str, Any]:
    if not target.is_file():
        raise SystemExit(f"gate1 sidecar must exist before Phase F enable: {V3K_GUI_SIDECAR_FILE}")
    loaded = load_v3k_gui_sidecar_file(target)
    if not loaded.valid:
        raise SystemExit("existing sidecar is invalid; refusing Phase F enable: " + "; ".join(loaded.diagnostics))
...
def _build_phase_f_payload(target: Path) -> dict[str, Any]:
    payload = _load_base_payload(target)
    settings = v3k_settings_defaults()
    existing = payload.get("settings")
    if isinstance(existing, dict):
        settings.update(existing)
    settings[FLAG_PHASE_F_ANALYZER_STRATEGY] = True
    payload["settings"] = settings
    payload["approval_state"] = "approved-gate2-phase-f-enabled"
    payload["approval_gate"] = PHASE_F_GATE
    payload["phase_f_approval_record"] = _approval_record()
    payload["phase_f_live_order_exit_wiring"] = False
    payload["phase_f_operating_database_written"] = False
    return payload


def _assert_phase_f_payload(payload: dict[str, Any]) -> tuple[str, ...]:
    result = validate_v3k_gui_sidecar_payload(payload)
    if not result.valid:
        raise SystemExit("Phase F sidecar payload failed validation: " + "; ".join(result.diagnostics))
    ...
    if unexpected:
        raise SystemExit(f"Phase F enable may only turn on {sorted(ALLOWED_ENABLED)}; got {unexpected}")
    if FLAG_PHASE_F_ANALYZER_STRATEGY not in enabled:
        raise SystemExit("Phase F analyzer strategy flag was not enabled in sidecar")
    return enabled


def write_phase_f_enable(*, approval_phrase: str) -> PhaseFEnableWriteResult:
    _assert_gate_authority(approval_phrase)
    _assert_forbidden_status_clean()
    target = ROOT / V3K_GUI_SIDECAR_FILE
    payload = _build_phase_f_payload(target)
    enabled = _assert_phase_f_payload(payload)
    ...
    if not idempotent:
        ...
        backup_path = backup_dir / f"v3k_gui_settings.phase-f-enable.{_timestamp()}.json"
        shutil.copy2(target, backup_path)
        tmp = target.with_name(f"{target.name}.phase-f.tmp")
        ...
        try:
            ...
            if target.read_text(encoding="utf-8") != before:
                raise RuntimeError("Phase F sidecar rollback invariant failed") from exc
            raise

    loaded = load_v3k_gui_sidecar_file(target)
    if not loaded.valid:
        raise RuntimeError(f"written Phase F sidecar failed validation: {loaded.diagnostics}")
    enabled_after = tuple(key for key, value in loaded.settings.items() if value)
    if enabled_after != enabled:
        raise RuntimeError(f"Phase F sidecar enabled set changed after write: {enabled_after} != {enabled}")
    _assert_forbidden_status_clean()
    return PhaseFEnableWriteResult(
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Approved writer for the V3K Phase F sidecar enable gate.")
    parser.add_argument("--approve", required=True, help="Exact Phase F gate approval phrase.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    result = write_phase_f_enable(approval_phrase=args.approve)
    if args.format == "json":
