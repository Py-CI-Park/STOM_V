...
공식 업데이트를 `research/*` 브랜치에 반영할 때는,
실제 적용을 실무 묶음 단위로 나누더라도 **커밋 기록은 공식 버전 단위로 남깁니다.**

- 커밋 제목 형식: `STOM V2.68`, `STOM V2.69`, ...
- 원칙: 공식 업데이트 1개 버전 = research 브랜치 커밋 1개 버전
- 의미:
  - 내부 적용 순서는 묶음 단위여도 됨
  - 최종 기록은 canonical base가 반영한 공식 버전 이름과 맞춰야 함
  - 연구 브랜치도 앞으로는 다른 워크트리와 같은 버전 추적 단위를 사용함
