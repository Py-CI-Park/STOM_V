...
- `STOM_Version_2U`? ?? V2? `.pyd`? `.py` ????? ???? ??/?? ????.
- `.pyd` ??? ?? ???? ??? ????? upstream? ???? ??, ??? ???? 2U ??/?? ??? ??? ????.
- `.pyd` ???? ?? `.py` ???? ??? ????. ??? `set_*.py`, `ui_button_clicked_*.py`, `ui_activated_*.py`? ???? MainWindow ???/??/?? ??? ????.
- ?? ??? ???? ?? GUI ????? ?? ??/??? ??, ????/?? ?????, ???? `activated` wrapper? ?? ????.
- `sactivated_*`/`cactivated_*`?? ?? ??? ?? alias ??? pyd ?? ???. ?? ??? `activated_XX(self, 'stock'/'coin')` ??? ????.
- X ??, ??? ??, ???? ?? ??? `.pyd` MainWindow? ???? ???? ??? ?? ?? ??? ????.
- ??? ??? 2U_C?? ?? ?? ??? ???? ????.
...
- 2U에서는 V2.36.U1.5에서 시리얼키 기능을 **의도적으로 제거**함
- pyd→py 추론 시 **시리얼키 관련 코드를 절대 추가하지 않음**
- 시리얼키 관련 업스트림 변경 (인증, 만료일, QThread 변환 등)은 추론에서 **제외**
