...
**이 문서가 프로젝트의 개발 명세서이자 실행 계획서입니다.**
모든 개발 작업은 이 문서에 정의된 34단계(V1U.00 ~ V1U.33)를 순차적으로 따라야 합니다.
...
| 4 | **순차 개발 필수** | V1U.00 → V1U.01 → ... → V1U.33 순서로만 진행 |
| 5 | **각 단계 독립 커밋** | 매 단계마다 `STOM V1U.XX` 형식으로 커밋 |
| 6 | **V2 커밋 diff 참조 개발** | 각 단계에서 해당 V2 커밋의 diff를 분석하여 이해 기반으로 재구현 |
...
V1U가 V2.33까지 개발 완료되면 다음 변경이 반영됩니다:
...
예시:
```
STOM V1U.00 - 키움증권 해외선물 추가 및 인증 시스템

1. future/ 폴더 생성 및 해외선물 모듈 구현
2. 인증 시스템 추가 (시리얼키 검증)
3. 스톰 라이브 ZMQ 방식 전환
4. strftime/strptime 전면 수정
5. ui_mainwindow.py 해외선물 UI 요소 추가
```
