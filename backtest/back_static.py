...
TRADE_RESULT_B_COLUMNS = [
    ...
    # v2 확장(QSP1 P1, 2026-07-30) — 매수시점 상태를 라벨 데이터셋(수익률=라벨)의
    #   설명변수로 쓰기 위한 추가 스냅샷. 캡처는 _capture_prefixed_snapshot 이 이름
    #   기반(dict_findex)으로 하므로 여기 이름 추가만으로 끝난다. 타임프레임에 없는
    #   필드(예: min 에서 초당*)는 default 0 으로 기록되고, 분석기가 zero-variance
    #   컬럼으로 자동 제외한다. 구버전 행은 normalize_trade_result_rows 가 0-패딩.
    # -- 공통(tick/min 모두 존재): 당일 OHL·전일비·위치/각도·체결강도평균 --
    'B_시가', 'B_고가', 'B_저가', 'B_전일비', 'B_고저평균대비등락율',
    'B_체결강도평균', 'B_등락율각도', 'B_당일거래대금각도',
    # -- tick 전용(min 에서는 0) --
    'B_초당거래대금', 'B_초당거래대금평균', 'B_누적초당매수수량', 'B_누적초당매도수량',
    # -- min 전용(tick 에서는 0) --
    'B_분당거래대금', 'B_분당거래대금평균', 'B_분당매수수량', 'B_분당매도수량', 'B_RSI',
]
