...
from utility.setting import dgree, BACK_TEMP
from utility.static import dt_ymdhm, pickle_read
                ...
                for vkey in self.trade_info[vturn].keys():
                    ...
                    if '매수' in gubun:
                        ...
                        if not 보유중:
                            ...
                            if not self.CheckDividBuy(포지션, 현재가, 추가매수가, 수익율, vturn, vkey) and self.dict_set['주식매수분할시그널']:
                                exec(self.buystg)
                    ...
                    if '매도' in gubun:
                        ...
                        if self.dict_set['주식매도분할횟수'] == 1:
                            ...
                            if not self.CheckDividSell(포지션, 수익율, 매도분할횟수, vturn, vkey) and self.dict_set['주식매도분할시그널']:
                                exec(self.sellstg)
