    ...
    def Strategy(self):
        ...
        if self.dict_condition:
            ...
            for k, v in self.dict_condition.items():
                exec(guard_exec_code(v, f'BackEngineBinanceTick2.condition.{k}'))
            ...
            for vturn in self.trade_info:
                ...
                for vkey in self.trade_info[vturn]:
                    ...
                    if '매수' in gubun:
                        ...
                        if not 보유중:
                            exec(guard_exec_code(self.buystg, 'BackEngineBinanceTick2.buystg'))
                        else:
                    ...
                    if '매도' in gubun:
                        ...
                        if self.dict_set['코인매도분할횟수'] == 1:
                            exec(guard_exec_code(self.sellstg, 'BackEngineBinanceTick2.sellstg'))
                        else:
                    ...
                    if '매수' in gubun:
                        ...
                        if not 보유중:
                            if self.back_type != '조건최적화':
                                ...
                                exec(guard_exec_code(self.dict_buystg[index_], f'BackEngineBinanceTick2.dict_buystg.{index_}'))
                        else:
                            if not self.CheckDividBuy(포지션, 현재가, 추가매수가, 수익률, vturn, vkey) and self.dict_set['코인매수분할시그널']:
                                if self.back_type != '조건최적화':
                                    exec(guard_exec_code(self.buystg, 'BackEngineBinanceTick2.buystg'))
                    ...
                    if '매도' in gubun:
                        ...
                        if self.dict_set['코인매도분할횟수'] == 1:
                            if self.back_type != '조건최적화':
                                ...
                                exec(guard_exec_code(self.dict_sellstg[index_], f'BackEngineBinanceTick2.dict_sellstg.{index_}'))
                        else:
                            if not self.CheckDividSell(포지션, 수익률, 매도분할횟수, vturn, vkey) and self.dict_set['코인매도분할시그널']:
                                if self.back_type != '조건최적화':
                                    exec(guard_exec_code(self.sellstg, 'BackEngineBinanceTick2.sellstg'))
