...
from utility.static import timedelta_sec, roundfigure_upper, roundfigure_lower, GetKiwoomPgSgSp, GetUvilower5, \
    dt_ymdhms, dt_ymdhm, GetHogaunit
...
class BackEngineKiwoomTick2(BackEngineKiwoomTick):
    ...
    def Strategy(self):
        ...
        if self.opti_turn == 1:
            for vturn in self.trade_info:
                ...
                for vkey in self.trade_info[vturn]:
                    ...
                    if '매수' in gubun:
                        ...
                        self.SetBuyCount2(vturn, vkey, 보유중, 매수가, 현재가, 저가대비고가등락율, 순매수금액, 당일거래대금,
                    ...
                    if '매도' in gubun:
                        ...
                        if self.CancelSellOrder(현재가, 매수분할횟수, vturn, vkey): continue
            ...
            if '매도' in gubun:
                ...
                if self.CancelSellOrder(현재가, 매수분할횟수, vturn, vkey): return
                self.SetSellCount2(vturn, vkey, 보유수량, 현재가, 저가대비고가등락율, 순매수금액, 당일거래대금,
                                   매도분할횟수, 매도호가1, 매수호가1, 호가단위)
                if self.dict_set['주식매도분할횟수'] == 1:
                    exec(self.sellstg)
    ...
    def SetBuyCount2(self, vturn, vkey, 보유중, 매수가, 현재가, 저가대비고가등락율, 순매수금액, 당일거래대금, 매수분할횟수, 매도호가1, 매수호가1, 호가단위):
    ...
    def CheckDividBuy(self, 현재가, 추가매수가, 수익률, vturn, vkey):
        분할매수기준수익률 = np.round((현재가 / 추가매수가 - 1) * 100, 2) if self.dict_set['주식매수분할고정수익률'] else 수익률
    ...
    def CancelSellOrder(self, 현재가, 매수분할횟수, vturn, vkey):
        cancel = False
        ...
        if 주문수량 > 0:
            if self.dict_set['주식매수주문구분'] == '시장가':
                ...
                if 미체결수량 <= 0:
                    ...
                    추가매수가 = int(np.round(매수금액 / 주문수량))
                    보유수량 = 직전보유수량 + 주문수량
                    매수가 = int(np.round((직전매수가 * 직전보유수량 + 매수금액) / 보유수량))
                ...
                self.trade_info[vturn][vkey]['매수호가단위'] = \
                    self.dict_arry[self.indexn, 25] - self.dict_arry[self.indexn, 26]
        ...
        if self.dict_set['주식매도주문구분'] == '시장가':
            ...
            for 매수호가, 매수잔량 in self.shogainfo:
                if 미체결수량 - 매수잔량 <= 0:
                    ...
                    미체결수량 -= 매수잔량
            if 미체결수량 <= 0:
                self.trade_info[vturn][vkey]['매도가'] = int(np.round(매도금액 / 주문수량))
