...
class BackEngineKiwoomMin2(BackEngineKiwoomTick2):
    def Strategy(self):
        ...
        self.mc = arry_indi[:, self._fi('현재가')]
        self.mh = arry_indi[:, self._fi('분봉고가')]
        self.ml = arry_indi[:, self._fi('분봉저가')]
        self.mv = arry_indi[:, self._fi('분당거래대금')]
                ...
                for vkey in self.trade_info[vturn]:
                    ...
                    if '매수' in gubun:
                        ...
                        if not 보유중:
                            ...
                            if not self.CheckDividBuy(포지션, 현재가, 추가매수가, 수익률) and self.dict_set['주식매수분할시그널']:
                                exec(self.buystg)
                ...
                for vkey in self.trade_info[vturn]:
                    ...
                    if '매수' in gubun:
                        ...
                        if not 보유중:
                            ...
                            if not self.CheckDividBuy(포지션, 현재가, 추가매수가, 수익률) and self.dict_set['주식매도분할시그널']:
                                if self.back_type != '조건최적화':
                                    exec(self.buystg)
                                else:
                                    exec(self.dict_buystg[index_])
