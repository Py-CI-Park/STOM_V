            ...
            for vturn in self.trade_info:
                ...
                for vkey in self.trade_info[vturn]:
                    ...
                    if self.indistg is not None:
                        exec(guard_exec_code(self.indistg, 'BackEngineBinanceMin.indistg'))
                    ...
                    if self.dict_condition:
                        ...
                        for k, v in self.dict_condition.items():
                            exec(guard_exec_code(v, f'BackEngineBinanceMin.condition.{k}'))
                    ...
                    if not self.trade_info[vturn][vkey]['보유중']:
                        ...
                        exec(guard_exec_code(self.buystg, 'BackEngineBinanceMin.buystg'))
                    else:
                        ...
                        exec(guard_exec_code(self.sellstg, 'BackEngineBinanceMin.sellstg'))
                ...
                for vkey in self.trade_info[vturn]:
                    ...
                    if not self.trade_info[vturn][vkey]['보유중']:
                        ...
                        if self.back_type != '조건최적화':
                            ...
                            exec(guard_exec_code(self.dict_buystg[index_], f'BackEngineBinanceMin.dict_buystg.{index_}'))
                    else:
                        수익률, 최고수익률, 최저수익률, 보유수량, 보유시간, 매수틱번호 = self.SetSellCount(vturn, vkey, 현재가, now())
                        포지션 = 'LONG' if self.trade_info[vturn][vkey]['보유중'] == 1 else 'SHORT'
                        if self.back_type != '조건최적화':
                            ...
                            exec(guard_exec_code(self.dict_sellstg[index_], f'BackEngineBinanceMin.dict_sellstg.{index_}'))
        else:
            vturn, vkey = 0, 0
