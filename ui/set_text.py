...
평균값계산틱수 : 변수들의 평균, 최저, 최고 등의 값을 계산하는 최소 틱수 또는 봉수
...
VI해제시간(datetime), VI가격(int), VI호가단위(int), VI아래5호가(int), 호가단위(int), 데이터길이(int), 시분초(int)
종목명(str), 종목코드(str), 매수(True)
# 이전값 조회 시 봉수 입력은 1부터 데이터길이 - 1까지이다.
평균값계산틱수 : 변수들의 평균, 최저, 최고 등의 값을 계산하는 최소 틱수 또는 봉수
매수 : True 상태의 변수이다. 전략 상단에 매수 = False로 시작하면 참인 조건들을 and로 묶어서 전략을 작성할 수 있다.
현재가 : 1분마다 기록된 봉의 종가
시가, 고가, 저가 : 일봉상 시고저가
...
# 이전값 조회 시 봉수 입력은 1부터 데이터길이 - 1까지이다. -1입력은 모든 변수의 매수시점정보를 조회한다.
"""'''
...
최고분봉고가 : 입력한 구간봉수 동안 분봉고가의 최고값
최저분봉저가 : 입력한 구간봉수 동안 분봉저가의 최저값
분당거래대금 : 1분 동안 거래대금의 누적값(단위:USDT)
분당거래대금평균 : 입력한 구간봉수 동안 분당거래대금의 평균값
체결강도 : 매도수량 대비 매수수량의 비율, 최소값0, 최대값500, 매도수량이 없을 경우와 비율이 500이 넘을 경우 500 (매수수량 / 매도수량 * 100)
체결강도평균 : 입력한 구간봉수 동안 체결강도의 평균값
최고체결강도 : 입력한 구간봉수 동안 체결강도의 최고값
...
최고현재가 : 입력한 구간틱수 동안 현재가의 최고값
최저현재가 : 입력한 구간틱수 동안 현재가의 최저값
초당거래대금 : 1초 동안 거래대금의 누적값(단위:USDT)
초당거래대금평균 : 입력한 구간틱수 동안 초당거래대금의 평균값
체결강도 : 매도수량 대비 매수수량의 비율, 최소값0, 최대값500, 매도수량이 없을 경우와 비율이 500이 넘을 경우 500 (매수수량 / 매도수량 * 100)
체결강도평균 : 입력한 구간틱수 동안 체결강도의 평균값
최고체결강도 : 입력한 구간틱수 동안 체결강도의 최고값
...
# 변수명 뒤에 'N(이전틱수)'을 붙여 이전값을 조회할 수 있다 예: 현재가N(1) - 1틱전 현재가
현재가(float), 시가(float), 고가(float), 저가(float), 등락율(float), 고저평균대비등락율(float), 당일거래대금(int), 체결강도(float),
초당매수수량(int), 초당매도수량(int), 초당거래대금(int), 매도총잔량(int), 매수총잔량(int), 매도호가1~5(float), 매도잔량1~5(int),
매수호가1~5(float), 매수잔량1~5(int), 매도수5호가잔량합(int)
# 구간연산 변수들 - 괄호 안에 '변수명(구간틱수, 이전틱수)' 형태로 사용한다.
# 이전틱수 미입력 시 현재값을 호출한다. 예 : 이동평균(60). 이동평균(60, 1)
이동평균(float), 최고현재가(float), 최저현재가(float), 초당거래대금평균(float), 체결강도평균(float), 최고체결강도(float), 최저체결강도(float),
누적초당매수수량(int), 누적초당매도수량(int), 최고초당매수수량(int), 최고초당매도수량(int), 당일거래대금각도(float), 등락율각도(float)
# 그외 변수들
호가단위(float), 데이터길이(int), 시분초(int), 종목코드(str), BUY_LONG(True), SELL_SHORT(True)
# 이전값 조회 시 틱수 입력은 1부터 데이터길이 - 1까지이다.
평균값계산틱수 : 변수들의 평균, 최저, 최고 등의 값을 계산하는 최소 틱수 또는 봉수
...
# 변수명 뒤에 'N(이전봉수)'을 붙여 이전값을 조회할 수 있다 예: 현재가N(1) - 1봉전 현재가
현재가(float), 시가(float), 고가(float), 저가(float), 등락율(float), 고저평균대비등락율(float), 당일거래대금(int), 체결강도(float),
분당매수수량(int), 분당매도수량(int), 분당거래대금(int), 매도총잔량(int), 매수총잔량(int), 매도호가1~5(float), 매도잔량1~5(int),
매수호가1~5(float), 매수잔량1~5(int), 매도수5호가잔량합(int), 분봉시가(float), 분봉고가(float), 분봉저가(float)
# 구간연산 변수들 - 괄호 안에 '변수명(구간봉수, 이전봉수)' 형태로 사용한다.
# 이전봉수 미입력 시 현재값을 호출한다. 예 : 이동평균(60). 이동평균(60, 1)
이동평균(float), 최고현재가(float), 최저현재가(float), 분당거래대금평균(float), 체결강도평균(float), 최고체결강도(float), 최저체결강도(float),
누적분당매수수량(int), 누적분당매도수량(int), 최고분당매수수량(int), 최고분당매도수량(int), 당일거래대금각도(float), 등락율각도(float),
최고분봉고가(float), 최고분봉저가(float)
# 보조지표, 변수명 뒤에 '_N(이전봉수)'을 붙여 이전값을 조회할 수 있다.
AD, ADOSC, ADXR, APO, AROOND, AROONU, ATR, BBU, BBM, BBL, CCI, DIM, DIP, MACD, MACDS, MACDH, MFI, MOM,
OBV, PPO, ROC, RSI, SAR, STOCHSK, STOCHSD, STOCHFK, STOCHFD, WILLR
# 그외 변수들
...
체결강도차이 = np.round(체결강도 - 체결강도평균(30), 2)
누적수량비 = np.round(누적초당매수수량(30) / 누적초당매도수량(30), 2) if 누적초당매도수량(30) > 0 else 0
초당순매수금액 = int((초당매수수량 - 초당매도수량) * 현재가 / 1_000_000)
매도총잔량대비매수수량비율 = np.round(초당매수수량 / 매도총잔량 * 100, 2) if 매도총잔량 > 0 else 0
당일거래대금대비초당거래대금평균비율 = np.round(초당거래대금평균(30) / 당일거래대금 * 100, 2)
...
탐색범위고가 = self.dict_arry[self.indexn + 1:self.indexn + 1 + 탐색틱수, 1].max()
현재가대비고가등락율 = np.round((탐색범위고가 / 현재가 - 1) * 100, 2)
...
try:
    마지막날짜 = str(self.dict_arry[self.indexn + 탐색틱수, 0])[:8]
except:
    마지막날짜 = ''
