...
def get_label_text(ui, real, gubun, code, is_min, xpoint, factor, hms):
    ...
    if factor == '현재가':
        if gubun == 'S':
            ...
            if is_min:
                text = f"시간 {hms}\n" \
                       ...
                       f"분봉시가 {ui.ctpg_arry[xpoint, fi('분봉시가')]:,.8f}\n" \
                       f"분봉고가 {ui.ctpg_arry[xpoint, fi('분봉고가')]:,.8f}\n" \
                       f"분봉저가 {ui.ctpg_arry[xpoint, fi('분봉저가')]:,.8f}\n" \
                       f"현재가    {ui.ctpg_arry[xpoint, fi('현재가')]:,.8f}"
            else:
                text = f"시간 {hms}\n" \
        ...
        if gubun in ('S', 'F'):
            text = f"초당매수수량 {ui.ctpg_arry[xpoint, fi('초당매수수량')]:,.0f}\n" \
                   f"초당매도수량 {ui.ctpg_arry[xpoint, fi('초당매도수량')]:,.0f}"
        else:
            text = f"초당매수수량 {ui.ctpg_arry[xpoint, fi('초당매수수량')]:,.8f}\n" \
                   f"초당매도수량 {ui.ctpg_arry[xpoint, fi('초당매도수량')]:,.8f}"
    elif factor == '분당체결수량':
        if gubun in ('S', 'F'):
            text = f"분당매수수량 {ui.ctpg_arry[xpoint, fi('분당매수수량')]:,.0f}\n" \
                   f"분당매도수량 {ui.ctpg_arry[xpoint, fi('분당매도수량')]:,.0f}"
        else:
            text = f"분당매수수량 {ui.ctpg_arry[xpoint, fi('분당매수수량')]:,.8f}\n" \
                   f"분당매도수량 {ui.ctpg_arry[xpoint, fi('분당매도수량')]:,.8f}"
    elif factor == '호가총잔량':
        if gubun in ('S', 'F'):
            text = f"매도총잔량 {ui.ctpg_arry[xpoint, fi('매도총잔량')]:,.0f}\n" \
                   f"매수총잔량 {ui.ctpg_arry[xpoint, fi('매수총잔량')]:,.0f}"
        else:
            text = f"매도총잔량 {ui.ctpg_arry[xpoint, fi('매도총잔량')]:,.8f}\n" \
                   f"매수총잔량 {ui.ctpg_arry[xpoint, fi('매수총잔량')]:,.8f}"
    elif factor == '매도수호가잔량1':
        if gubun in ('S', 'F'):
            text = f"매도1잔량 {ui.ctpg_arry[xpoint, fi('매도잔량1')]:,.0f}\n" \
                   f"매수1잔량 {ui.ctpg_arry[xpoint, fi('매수잔량1')]:,.0f}"
        else:
            text = f"매도1잔량 {ui.ctpg_arry[xpoint, fi('매도잔량1')]:,.8f}\n" \
                   f"매수1잔량 {ui.ctpg_arry[xpoint, fi('매수잔량1')]:,.8f}"
    elif factor == '매도수5호가잔량합':
        if gubun in ('S', 'F'):
            text = f"매도수5호가잔량합 {ui.ctpg_arry[xpoint, fi('매도수5호가잔량합')]:,.0f}"
        else:
            text = f"매도수5호가잔량합 {ui.ctpg_arry[xpoint, fi('매도수5호가잔량합')]:,.8f}"
    elif factor == '누적초당매도수수량':
        if gubun in ('S', 'F'):
            text = f"누적초당매수수량 {ui.ctpg_arry[xpoint, fi('누적초당매수수량')]:,.0f}\n" \
                   f"누적초당매도수량 {ui.ctpg_arry[xpoint, fi('누적초당매도수량')]:,.0f}"
        else:
            text = f"누적초당매수수량 {ui.ctpg_arry[xpoint, fi('누적초당매수수량')]:,.8f}\n" \
                   f"누적초당매도수량 {ui.ctpg_arry[xpoint, fi('누적초당매도수량')]:,.8f}"
    elif factor == '누적분당매도수수량':
        if gubun in ('S', 'F'):
            text = f"누적분당매수수량 {ui.ctpg_arry[xpoint, fi('누적분당매수수량')]:,.0f}\n" \
                   f"누적분당매도수량 {ui.ctpg_arry[xpoint, fi('누적분당매도수량')]:,.0f}"
        else:
            text = f"누적분당매수수량 {ui.ctpg_arry[xpoint, fi('누적분당매수수량')]:,.8f}\n" \
                   f"누적분당매도수량 {ui.ctpg_arry[xpoint, fi('누적분당매도수량')]:,.8f}"
    elif factor == 'AROON':
        text =     f"AROOND {ui.ctpg_arry[xpoint, fi('AROOND')]:,.2f}\n" \
                   f"AROONU {ui.ctpg_arry[xpoint, fi('AROONU')]:,.2f}"
    elif factor == 'BBAND':
        text =     f"BBU {ui.ctpg_arry[xpoint, fi('BBU')]:,.2f}\n" \
                   f"BBM {ui.ctpg_arry[xpoint, fi('BBM')]:,.2f}\n" \
                   f"BBL {ui.ctpg_arry[xpoint, fi('BBL')]:,.2f}"
    elif factor == 'MACD':
        text =     f"MACD   {ui.ctpg_arry[xpoint, fi('MACD')]:,.2f}\n" \
                   f"MACDS {ui.ctpg_arry[xpoint, fi('MACDS')]:,.2f}\n" \
                   f"MACDH {ui.ctpg_arry[xpoint, fi('MACDH')]:,.2f}"
    elif factor == 'DMI':
        text =     f"DIM {ui.ctpg_arry[xpoint, fi('DIM')]:,.2f}\n" \
                   f"DIP {ui.ctpg_arry[xpoint, fi('DIP')]:,.2f}"
    elif factor == 'STOCHS':
        text =     f"STOCHSK {ui.ctpg_arry[xpoint, fi('STOCHSK')]:,.2f}\n" \
                   f"STOCHSD {ui.ctpg_arry[xpoint, fi('STOCHSD')]:,.2f}"
    elif factor == 'STOCHF':
        text =     f"STOCHFK {ui.ctpg_arry[xpoint, fi('STOCHFK')]:,.2f}\n" \
    ...
    if not real and ui.dict_fn is not None and factor in ui.dict_fn:
        for name, flist in ui.dict_fn[factor].items():
            ...
            if ftype in ('선:일반', '선:조건'):
                text = f"{text}\n{name} {ui.ctpg_arry[xpoint, findex]:,}"
            else:
