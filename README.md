...
> **프로젝트 총괄 문서 (3/3)**: 이 문서는 `CLAUDE.md`, `AGENTS.md`와 함께 프로젝트를 총괄합니다.
...
STOM V1 코드베이스를 기반으로, V2에서 순차적으로 개발된 37개 커밋(V2.00 ~ V2.36)의 변경사항을 **코드 이해 기반으로 재구현**합니다.
...
Initial commit (87aee04)
  ...
  └── STOM V1 (80ab4ec)
        ...
        └── STOM_Version_2   (V2.00 ~ V2.36, pyd 포함)
```
...
STOM_V/
├── stom.py              # 진입점
├── README.md            # ★ 프로젝트 총괄 (이 파일)
├── CLAUDE.md            # ★ AI 작업 지침
├── AGENTS.md            # ★ AI 아키텍처 가이드
│
├── docs/
│   ├── README.md        # ★ 문서 허브 (상위 3개 문서가 관리)
│   └── dev_plan/
...
| 단계 | V2 대응 | 내용 | 난이도 |
|------|---------|------|--------|
| V1U.00 | V2.00 | 키움증권 해외선물 추가, 인증 시스템, ZMQ 라이브 | 최고 |
| V1U.02 | V2.02 | DataFrame → Dictionary 전환 | 상 |
| V1U.11 | V2.11 | UI 파일 대규모 리네이밍 (26개 파일) | 최고 |
| V1U.19 | V2.19 | 리시버→에이전트 리네이밍, PyQT 이벤트루프 전환 | 최고 |
| V1U.23 | V2.23 | 리시버공유 코드 대폭 삭제 | 상 |
| V1U.26 | V2.26 | 리시버 공유 모드 완전 삭제 | 상 |
| V1U.34 | V2.34 | 보유시간 조건, 차트창, 보조지표 오류 수정 | 중 |
| V1U.36 | V2.36 | MERGE값 합산 방법 변경 (곱셈→덧셈) | 중 |
...
이 3개 문서가 프로젝트 전체와 `docs/README.md`를 관리합니다:
...
| 문서 | 역할 | 관리 주체 |
|------|------|----------|
| [`docs/README.md`](docs/README.md) | docs 폴더 문서 총괄 | **상위 3개 문서가 관리** |
| `docs/dev_plan/*.md` | 개발 계획 문서 | docs/README.md |

### 문서 관리 규칙

```
1. 새 문서는 docs/ 디렉토리 내 적절한 하위 폴더에 배치
2. docs/README.md에 새 문서 정보 등록
3. 필요시 상위 3개 문서(README.md, AGENTS.md, CLAUDE.md)에 참조 추가
```

> **개발 계획서**: `docs/dev_plan/STOM_Version_1U_Development_Plan.md`는 이 프로젝트의 **핵심 문서**입니다. 모든 개발 작업은 이 문서에 정의된 37단계 절차를 따릅니다.
