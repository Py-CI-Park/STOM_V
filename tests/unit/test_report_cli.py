...
def _report_env(backtest_db_path):
    return {
        **os.environ,
        'STOM_CLI_DB_BACKTEST': backtest_db_path,
    }
...
class TestReportBacktestJson:
    def test_backtest_json_has_data(self, tmp_backtest_db):
        result = subprocess.run(
    ...
    def test_backtest_json_with_limit(self, tmp_backtest_db):
        result = subprocess.run(
    ...
    def test_backtest_summary_json(self, tmp_backtest_db):
        result = subprocess.run(
            [PYTHON, STOM_BACKTEST, 'report', '--source', 'backtest',
             '--format', 'json', '--summary'],
            capture_output=True, text=True, timeout=30, cwd=PROJECT_ROOT,
...
class TestReportCsv:
    def test_backtest_csv_creates_file(self, tmp_backtest_db):
