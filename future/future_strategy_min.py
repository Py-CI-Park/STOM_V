    ...
    def Strategy(self, data):
        ...
        if 종목코드 in self.dict_arry:
            ...
            if len_array >= 평균값계산틱수 - 1:
                ...
                등락율각도_      = round(math.atan2((등락율 - self.dict_arry[종목코드][-(평균값계산틱수 - 1), 5]) * dgree['stock']['min'][0], 평균값계산틱수) / (2 * math.pi) * 360, 2)
                당일거래대금각도_ = round(math.atan2((당일거래대금 - self.dict_arry[종목코드][-(평균값계산틱수 - 1), 6]) * dgree['stock']['min'][1], 평균값계산틱수) / (2 * math.pi) * 360, 2)
            ...
            if 체결강도평균_ != 0:
                ...
                if SBT and (A or B or (C and D) or (C and E) or D or E or F or G or H or J or K or L or M or N):
                    ...
                    if A or B or H or J or K or L or M or N:
                        ...
                        매도수량 = self.SetSellCount(분할매도횟수, 보유수량, 매입가, 저가대비고가등락율, 순매수금액, 당일거래대금, 등락율각도(30))
