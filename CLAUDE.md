...
### 사이클 종료 시 next steps 갱신 의무

각 사이클(시각 검증 + 자율 작업 묶음) 종료 시 `docs/V3U_NEXT_STEPS.md`를 갱신한다.

- §2 현재 사이클 상태 표 숫자 갱신 (결함 수, pytest 케이스, baseline 등)
- §5 선택 이력에 본 사이클 항목 추가 (사용자 선택, commit hash, 발견 결함)
- §3 옵션 카탈로그에 신규 옵션 추가 또는 제거된 옵션 정리
- §4 우선순위 매트릭스 재평가

본 문서는 V3U lane의 **미래 결정 진실 원천**이다. INFERENCE_LESSONS.md(과거 결함 기록)와 한 쌍으로 운영된다.
