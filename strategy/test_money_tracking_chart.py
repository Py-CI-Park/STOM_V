...
class MoneyTrackingChart(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 실시간 가격 그래프 (상단)
        self.realtime_plot = pg.PlotWidget(title="실시간 가격 그래프")
        ...
        # 그래프 추가 (현재가 영역 너비 줄임)
        graph_layout.addWidget(self.buy_plot, stretch=2)
        graph_layout.addWidget(self.price_plot, stretch=1)
        graph_layout.addWidget(self.sell_plot, stretch=2)
        
        main_layout.addWidget(self.realtime_plot)
    ...
    def setup_plot_styles(self):
        ...
        self.realtime_plot.setTitle("실시간 가격 그래프", color="black", size="12pt")
        
        # 막대그래프들
        for plot in [self.buy_plot, self.price_plot, self.sell_plot]:
