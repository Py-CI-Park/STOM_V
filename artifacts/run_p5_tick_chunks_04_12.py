...
def write_receipt(chunk_no: int, run: dict, rows: list[dict], prepare_line: str | None):
    ...
    summary = {
        ...
        "process_surface_clean": "verified by per-process return and final post-sequence scan required before G010 checkpoint",
        "next_allowed_action": (
...
def update_boulder(chunk_no: int, out: pathlib.Path, summary: dict) -> None:
    ...
    data["active_work"] = active
    BOULDER.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def main() -> int:
    print("[ORCH] remaining official tick chunks start chunks=05-12", flush=True)
    for chunk_no in range(5, 13):
    ...
    print("[ORCH] remaining official tick chunks complete chunks=05-12", flush=True)
    return 0
