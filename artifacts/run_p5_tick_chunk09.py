...
CHUNK_NO = 9
RUN_ID = "lat_tick_official_full_warm64_chunk09_20260704"
PAIRS = "docs/research/condition_research/research_runs/seed_lattice_20260702/pairs_tick_official_full_warm64_chunk09_20260704.json"


def rows_for(run_id: str):
    conn = sqlite3.connect(f"file:{DB.as_posix()}?mode=ro", uri=True, timeout=5)
    conn.row_factory = sqlite3.Row
    run = conn.execute("SELECT run_id,status,started_at,finished_at,best_gen,best_score FROM runs WHERE run_id=?", (run_id,)).fetchone()
    rows = [dict(r) for r in conn.execute("SELECT gen_no,buy_name,sell_name,status,gate_passed,reason,csv_path,trade_count,mdd,profit,total_profit_pct,daily_avg_trades,payoff_ratio,give_back_rate,strategy_gist FROM generations WHERE run_id=? ORDER BY gen_no", (run_id,))]
    conn.close()
    return (dict(run) if run else None), rows


def write_receipt(run: dict, rows: list[dict], prepare_line: str | None):
    ...
    summary = {
        ...
        "next_allowed_action": "P5 official tick chunk10 only with --fail-fast-timeout",
    }
    out = BASE / "p5_tick_chunk09_official_full_warm64_20260704_receipt.json"
    out.write_text(json.dumps({
        ...
        "summary": summary,
        "rows": rows,
        "verdict": "chunk09_execution_clean_per_row_integrity; trading_quality_bad_all_gate_failed; continue_only_as_coverage_map_not_promotion",
        "forbidden": [
...
def main() -> int:
    ...
    print(f"[ORCH] chunk09 start run_id={RUN_ID}", flush=True)
    ...
    if rc != 0 or not run or run.get("status") != "complete" or len(rows) != 24 or error_rows:
        print(f"[ORCH] chunk09 stop rc={rc} run_status={None if not run else run.get('status')} rows={len(rows)} errors={error_rows}", flush=True)
        return 19
    out, summary = write_receipt(run, rows, prepare_line)
    event = {
        "ts": time.strftime("%Y-%m-%dT%H:%M:%S+09:00"),
        "work_id": "css-v7-repair-plan-c-plan-b-d-20260703",
        "event": "p5_official_tick_chunk09_complete",
        "status": "chunk10_required",
    ...
    chunks = " + ".join(f"P5-official-tick-chunk{i:02d}" for i in range(1, 10))
    for target in [data.get("active_work", {}), data.get("works", {}).get("css-v7-repair-plan-c-plan-b-d-20260703")]:
        if not target:
            continue
        target["current_priority"] = "P5-official-tick-chunk10"
        target["completed_range"] = "P5-root-cause-repair + P5-profile-audit + P5-official-tick-preflight + P5-full-run-protocol-review + P5-official-tick-pilot12 + " + chunks
        target["completed_at"] = time.strftime("%Y-%m-%dT%H:%M:%S+09:00")
        target["last_completed_priority"] = "P5-official-tick-chunk09"
        target["last_completed_at"] = target["completed_at"]
        target["last_evidence"] = str(out.relative_to(ROOT)).replace("\\", "/")
        target["next_priority"] = summary["next_allowed_action"]
        target["p5_blocker_status"] = "chunk09_complete_chunk10_required"
        target["blocked_at"] = target["completed_at"]
        target["blocked_priority"] = "P5-official-min-and-P6"
        target["blocker"] = f"Chunk09 completed cleanly as execution evidence ({summary['recorded_rows']}/24 rows, gate_passed={summary['gate_passed_count']}). Single-run tick 288 remains forbidden. Next allowed action: {summary['next_allowed_action']}; min, P6, and P7 remain blocked until official tick export exists."
    BOULDER.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"[ORCH] chunk09 receipt={out.relative_to(ROOT).as_posix()} rows={summary['recorded_rows']} ok={summary['status_counts'].get('ok', 0)} gate={summary['gate_passed_count']} mdd={summary['min_mdd']}..{summary['max_mdd']}", flush=True)
    return 0
