...
function BtTradePathTab({ baseUrl, onNavigate }) {
  ...
  // 페이지 25 — 분석 카드 뷰어. 선택된 job 의 부검 카드를 그 자리에서 연다.
  ...
  return <section className="panel tp-workbench" aria-labelledby="tp-title">
    ...
    <div className="panel-bd">
      ...
      {/* QSP10 도달 지도 — job 없이 라벨만으로 동작한다(엔진 실행 전 탐색 단계). */}
      <div className="tp-map-bar">
        <button className={"btn sm" + (showReachMap ? " active" : " ghost")}
                onClick={() => setShowReachMap(!showReachMap)}
                title="라벨 지도 위에서 조건을 즉시 평가합니다(페이지 22~24). 백테스트 job 이 없어도 열립니다.">
          {showReachMap ? "도달 지도 닫기" : "도달 지도 열기 (QSP10 · 페이지 22~24)"}
        </button>
        <button className={"btn sm" + (showAnalysisCard ? " active" : " ghost")}
                onClick={() => setShowAnalysisCard(!showAnalysisCard)}
                disabled={!jobId}
                title="선택한 job 의 부검 카드(근본원인·변이축)를 봅니다. 자율 루프가 읽는 것과 같은 카드입니다.">
          {showAnalysisCard ? "분석 카드 닫기" : "분석 카드 열기 (페이지 25)"}
        </button>
        <button className={"btn sm" + (showTransferLedger ? " active" : " ghost")}
