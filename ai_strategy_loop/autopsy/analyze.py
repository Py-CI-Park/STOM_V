...
# 진입시점(B_*) 컬럼 — 단일 출처는 backtest/back_static.py TRADE_RESULT_B_COLUMNS.
#   v5.13.3(QSP1 P1) 하드코딩 사본이 SSOT 와 어긋나는 사고를 막기 위해 import 로 전환.
#   (엔진 v2 확장 17컬럼이 자동 편입된다. import 실패 시에만 구 14컬럼 폴백.)
try:
    from backtest.back_static import TRADE_RESULT_B_COLUMNS as B_COLUMNS  # noqa: N811
except Exception:  # noqa: BLE001 - 엔진 패키지 불가 환경(경량 테스트)용 폴백.
    B_COLUMNS = [
...
B_TO_STOM_VAR = {
    ...
    "B_분봉고가": "분봉고가",
    "B_분봉저가": "분봉저가",
    # v2 확장(QSP1 P1) — B_<이름> 은 전부 동명 STOM 변수의 매수시점 스냅샷이다.
    "B_시가": "시가", "B_고가": "고가", "B_저가": "저가", "B_전일비": "전일비",
    "B_고저평균대비등락율": "고저평균대비등락율", "B_체결강도평균": "체결강도평균",
    "B_등락율각도": "등락율각도", "B_당일거래대금각도": "당일거래대금각도",
    "B_초당거래대금": "초당거래대금", "B_초당거래대금평균": "초당거래대금평균",
    "B_누적초당매수수량": "누적초당매수수량", "B_누적초당매도수량": "누적초당매도수량",
    "B_분당거래대금": "분당거래대금", "B_분당거래대금평균": "분당거래대금평균",
    "B_분당매수수량": "분당매수수량", "B_분당매도수량": "분당매도수량", "B_RSI": "RSI",
}
